...
void inputStreamClose(void * ctx) {
    // Nothing to close for input streams
    UNUSED(ctx);
}
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_initContextFromInputStream(
    ...
    LOGI("Attempting to initialize whisper context from input stream");
    context = whisper_init(&loader);
    if (context == NULL) {
        LOGW("Failed to initialize whisper context from input stream");
    } else {
        LOGI("Successfully initialized whisper context from input stream");
    }
...
static struct whisper_context *whisper_init_from_asset(
    ...
    if (ctx == NULL) {
        LOGW("Failed to initialize whisper context from asset");
    } else {
        LOGI("Successfully initialized whisper context from asset");
    }
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_initContextFromAsset(
    ...
    LOGI("Initializing whisper context from asset: %s", asset_path_chars);
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_initContext(
    ...
    LOGI("Initializing whisper context from file: %s", model_path_chars);
    context = whisper_init_from_file_with_params(model_path_chars, whisper_context_default_params());
    if (context == NULL) {
        LOGW("Failed to initialize whisper context from file: %s", model_path_chars);
    } else {
        LOGI("Successfully initialized whisper context from file");
    }
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_freeContext(
    ...
    UNUSED(thiz);
    if (context_ptr == 0) {
        LOGW("Attempt to free null whisper context");
        return;
    }
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_fullTranscribe(
    ...
    struct whisper_context *context = (struct whisper_context *) context_ptr;
    if (context == NULL) {
        LOGW("Attempt to transcribe with null whisper context");
        return;
    }
    ...
    LOGI("Transcribing audio data with %d samples using %d threads", audio_data_length, num_threads);
    ...
    if (whisper_full(context, params, audio_data_arr, audio_data_length) != 0) {
        ...
        LOGI("Successfully completed transcription");
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_getTextSegmentCount(
    ...
    struct whisper_context *context = (struct whisper_context *) context_ptr;
    if (context == NULL) {
        LOGW("Attempt to get text segment count with null whisper context");
        return 0;
    }
    int count = whisper_full_n_segments(context);
    LOGI("Found %d text segments", count);
    return count;
}
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_getTextSegment(
    ...
    struct whisper_context *context = (struct whisper_context *) context_ptr;
    if (context == NULL) {
        LOGW("Attempt to get text segment with null whisper context");
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_getTextSegmentT0(
    ...
    struct whisper_context *context = (struct whisper_context *) context_ptr;
    if (context == NULL) {
        LOGW("Attempt to get text segment t0 with null whisper context");
        return 0;
    }
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_getTextSegmentT1(
    ...
    struct whisper_context *context = (struct whisper_context *) context_ptr;
    if (context == NULL) {
        LOGW("Attempt to get text segment t1 with null whisper context");
        return 0;
    }
